...
if [ "$LETSENCRYPT_MODE" == "staging" ]; then
    ...
    certbot --config-dir "/spcwaterproof-certificates/$LETSENCRYPT_MODE" certonly --webroot -w "/spcwaterproof-certificates" -d "$HTTPS_HOST" -m "$ADMIN_EMAIL" --agree-tos --non-interactive --staging --server https://acme-v02.api.letsencrypt.org/directory
elif [ "$LETSENCRYPT_MODE" == "production" ]; then
    printf "\nTrying to get PRODUCTION certificate\n"
    certbot --config-dir "/spcwaterproof-certificates/$LETSENCRYPT_MODE" certonly --webroot -w "/spcwaterproof-certificates" -d "$HTTPS_HOST" -m "$ADMIN_EMAIL" --agree-tos --non-interactive --server https://acme-v02.api.letsencrypt.org/directory
else
