...
def consultar_articulo(request, id=0):
    ...
    try:
        ...
        references = None
        if Referencies.objects.filter(article=article.pk).exists():
            references=Referencies.objects.filter(article=article.pk)      
        enlaces = None
        enlaces=Links.objects.filter(articulo=article.pk)        
...
def consultar_categorias(request , categoria_id):
    ...
    articles=Article.objects.filter(categories=categoria_id)
...
def listar_articulos(request):
    articles=Article.objects.filter(publico='True')
