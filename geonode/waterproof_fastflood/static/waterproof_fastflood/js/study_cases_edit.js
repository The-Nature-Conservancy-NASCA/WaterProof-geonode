/**
 * @file Edit Study Case wizard step
 * validations & interactions
 * @version 1.0
 *
 * Note: Helper functions (getCookie, ajaxRequest, postData, getData, each,
 * createProgressTable, updateProgressTable) have been moved to study_cases_common.js
 */
...
$(document).ready(function () {
    //console.log("Document ready study case edit");
    ...
    if ($("#damage-table-body tr").length == 0){
        //console.log("Load default damage data");
        getDamagesByCountry(localStorage.countryCode);
    }
    ...
    $('#step1NextBtn').click(function () {
        ...
        watersheds = [];        
        validWatershed = true;
        const scNameList = JSON.parse(localStorage.getItem('studyCasesList'));        
    ...
    $('#step4NextBtn').click(function () {
        ...
        if (validateDamageData()) {
            ...
            .catch(error => {
                ...
                showError('Error', 'Failed to save step 4 data: ' + error.message);
            });
        }else{
                showWarning(gettext('field_empty'), gettext('error_fields'));
            return;
    ...
    $('#step5NextBtn').click(function () {
        ...
        if (valid) {
            ...
            .catch(error => {
                ...
                showError('Error', 'Failed to save step 5 data: ' + error.message);
            });
        } else {
            showWarning(gettext('field_empty'), gettext('error_fields'));
            return;
    ...
    $('#step6NextBtn').click(function () {
        ...
        if (nbs.length > 0) {
            ...
            .catch(error => {                
                console.error("Error saving NBS:", error);
                showError('Error', 'Failed to save NBS: ' + error.message);
            })
            .finally(() => {
                toggleLoadingStep("hide");
                try{
    ...
    function afterValidationStep7Run(valid_edit, valid_period){
        ...
        if ($('#period_analysis').val() != '' && $('#period_nbs').val() != '' && valid_edit && valid_period) {
            ...
            toggleLoadingStep("show")
            getData("/fastflood/currencys/", {id: id_study_case,currency: analysis_currency})
            .then(data => {
                    ...
                    showAlert({
                        ...
                        if (result.isConfirmed) {
                            ...
                            .catch(error => {
                                ...
                                showError('Error', 'Failed to save analysis: ' + error.message);
                            });
            ...
            .catch(error => {
                ...
                showError('Error', 'Failed to load currencies: ' + error.message);
            });
        } else {
            showWarning(gettext('field_empty'), gettext('error_fields'));
            return;
    ...
    function saveStudyCaseBiophysicalParams() {
        ...
        .catch(error => {
            ...
            showError('Error', 'Failed to save biophysical params: ' + error.message);
        });
    ...
    function saveDamageData() {
        ...
        .catch(error => {
            console.error("Error al guardar:", error);
            showError('Error', 'Failed to save damage data: ' + error.message);
        });
    }
    ...
    function fillCsvDamageData(id_study_case) {
        ...
        .then(data => {
            console.log("Create damage file successfully:", data);
        })
        .catch(error => {
    ...
    // Ensure height is adjusted after all initial content is loaded
    // Use setTimeout to allow all async operations to complete
    setTimeout(function() {
        //console.log("Final adjustment after page load");
        autoAdjustHeightTabContent();
    }, 500);
});
