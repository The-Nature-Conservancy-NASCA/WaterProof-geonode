...
$(document).ready(function () {
    console.log("Document ready study case create");
    ...
    $("#climate_year").prop("disabled", $("#analysis_nbs option:selected").text() == "HISTORIC");
    ...
    $('#step1NextBtn').click(function () {
        ...
        if (($('#name').val() != '' && $('#description').val() != '' && valid_intakes)) {
            ...
            .then(data => {
                ...
                if (id_study_case == '') {
                    ...
                    .then(result => {
                        ...
                        getDamagesByCountry();
                        loadStormDurations(watersheds[0]);
                        toggleLoadingStep("hide");
    ...
    $('#step2NextBtn').click(function () {
        ...
        .catch(error => {
            toggleLoadingStep("hide");
            console.error("Error saving carbon market data:", error);
            showError('Error', 'Failed to save carbon market data: ' + error.message);
        });
    });
    ...
    $('#step4NextBtn').click(function () {
        ...
        if (validateDamageData()) {
            ...
            .catch(error => {
                toggleLoadingStep("hide");
                console.error("Error saving biophysical data:", error);
                showError('Error', 'Failed to save biophysical data: ' + error.message);
            });
        }else{
                showWarning(gettext('field_empty'), gettext('error_fields'));
            return;
    ...
    $('#step5NextBtn').click(function () {
        ...
        if (valid) {
            ...
            .catch(error => {
                ...
                showError('Error', 'Failed to save financial data: ' + error.message);
            });
        } else {
            showWarning(gettext('field_empty'), gettext('error_fields'));
            return;
    ...
    $('#step6NextBtn').click(function () {
        ...
        if (nbs.length > 0) {
            ...
            .catch(error => {
                ...
                showError('Error', 'Failed to save NBS data: ' + error.message);
            });
        } else {
            showWarning(gettext('field_empty'), gettext('error_fields'));
            return;
                    ...
                    if (valid_nbs) {
                        showAlert({
                            ...
                            if (result.isConfirmed) {
                                ...
                                .catch(error => {
                                    console.error("Error saving investment scenario:", error);
                                    showError('Error', 'Failed to save investment scenario: ' + error.message);
                                });
    ...
    function saveDamageData() {
        ...
        rows.forEach(row => {
            const cells = row.querySelectorAll('td');
            const entry = {
                flood_depth: parseFloat(cells[0].innerText.trim()) || 0,
                residential: parseFloat(cells[1].innerText.trim()) || 0,
                commercial: parseFloat(cells[2].innerText.trim()) || 0,
                industrial: parseFloat(cells[3].innerText.trim()) || 0,
                infraroad: parseFloat(cells[4].innerText.trim()) || 0,
                agriculture: parseFloat(cells[5].innerText.trim()) || 0
            };
        ...
        depthRows.forEach(row => {
            const cells = row.querySelectorAll('td');
            const maxEntry = {
                residential: parseFloat(cells[0].innerText.trim()) || 0,
                commercial: parseFloat(cells[1].innerText.trim()) || 0,
                industrial: parseFloat(cells[2].innerText.trim()) || 0,
                infraroad: parseFloat(cells[3].innerText.trim()) || 0,
                agriculture: parseFloat(cells[4].innerText.trim()) || 0
            };
        ...
        .catch(error => {
            console.error("Error al guardar damage data:", error);
        });
    }
    ...
    function fillCsvDamageData(id_study_case) {
        ...
        $('#damage-table-body tr').each(function () {
            ...
            const obj = {
                "Flooddepth[m]": parseFloat($(cells[0]).text().trim()),
                "Residential": parseFloat($(cells[1]).text().trim()),
                "Commercial": parseFloat($(cells[2]).text().trim()),
                "Industrial": parseFloat($(cells[3]).text().trim()),
                "InfraRoads": parseFloat($(cells[4]).text().trim()),
                "Agriculture": parseFloat($(cells[5]).text().trim())
            };
            dataCurve.push(obj);
        });
        ...
        $('#max-damage-table-body tr').each(function () {
            ...
            const obj = {
                residential: parseFloat($(cells[0]).text().trim()),
                commercial: parseFloat($(cells[1]).text().trim()),
                industrial: parseFloat($(cells[2]).text().trim()),
                infra_roads: parseFloat($(cells[3]).text().trim()),
                agriculture: parseFloat($(cells[4]).text().trim())
            };
            dataMax.push(obj);
        });
        console.log(dataMax);
        console.log(dataMax[0]);
    ...
    function loadNBS() {
        ...
        .catch(error => {
            ...
            // Continue without NBS data for better UX
        });
    }

    // Note: loadStormDurations and buildStormDurationMessage functions
