...
$(function () {
    ...
    */
    activePlantGraph = function (ptapType) {
        ...
        $("[name=disableElement]").each(function (index, element) {
            ...
            if (modelInTypePtap) {
                listElements[element.getAttribute("plantElement")] = {
    ...
    * @returns 
    */
    viewBranch = function (e) {

        let elem = document.getElementById(e);
        if (elem.style.display === "none") {
            elem.style.display = "block";
            $(".main-point")[0].innerHTML = HYPHEN;
            $(".main-point").css("padding-left", "7px");
        } else {
            elem.style.display = "none";
            $(".main-point")[0].innerHTML = "+";
            $(".main-point").css("padding-left", "4px");
            $(".main-point").css("padding-top", "1px");
        }
    };
    /**
    ...
    addNewTechnology = function (parentId) {
        ...
        var textNewForm = `<div class="title-tree col-md-12" id="contentTechnology${idNewTech}">
            ...
            <div class="container-var" id="idContainerVar${idNewTech}"><div>
                ...
                <div class="help-block with-errors"></div></div></div>
            </div></div>${tableFunctionTpl}<div class="link-form" style="display:none;">${_('Add function')}</div></div>`;
                ...
                $.each(data, function (keyTech, valueTech) {
                    if (value.subprocessAddId === valueTech.subprocessAddId) {
                        if (lastTreeBranch.indexOf(valueTech.technology) === -1) {
                            ...
                            console.log("este es el tech name " + valueTech.technology);
                            var textTec = valueTech.technology;
                            if (valueTech.technology == "Quick mix") {
                                textTec = gettext("Mechanical mix");
                            };
                            var ht = `<div class="title-tree col-md-12" id="contentTechnology${techId}"> 
                                    ...
                                    <div class="text-tree"><label technology='${textTec}'>${lblTechnology}:</label> 
                                    ${_(textTec)}</div></div><div class="margin-main overflow-form col-md-12" id="technology${techId}"></div>`;
    ...
    */
    ...
    $(document).on('click', '.btn-function-cost', function () {
        ...
        let costFunction = {
            ...
            "description": elCheck.getAttribute("description") == 'undefined' ? elCheck.getAttribute("namefunction") : elCheck.getAttribute("description")
        };
    ...
    function showModalCalculator(flag, graphId, costFunction) {
        ...
        let desc = costFunction.description;
        desc = (desc == null || desc == "null") ? costFunction.name : desc;
        ...
        $('#costFunctionDescription').val(desc);
    ...
    $('#saveAndValideCost').click(function (e) {
        ...
        let description = $("#costFunctionDescription").val();

        if (fnName == "" || expression == "" || description == "" || factor == "") {
            ...
            selectedFunction4Edit.setAttribute('description', description);
            ...
            plant.functions[fnId].description = description;
            trElem.children[1].innerText = fnName;
            trElem.children[3].innerText = currency
            trElem.children[4].innerText = factor;

            if (plant.elements[selectedPlantElement].default){
                let fns = plant.elements[selectedPlantElement].default.filter(e => e.technology+"-"+e.costFunction == fnId);
                if (fns.length > 0){
                    fns.forEach(f =>{
                        f.currency = currency;
            ...
            arrayLoadingFunction.forEach(f => {
                if (f.functionTechnology +"-"+ f.functionName == fnId){
                    f.functionCurrency = currency;
                    f.functionValue = expression;
                    f.description = description;
                }
            })
    ...
    addNewFunction = function (techId, graphId, subprocess) {
        ...
        let description = $('#costFunctionDescription').val();
        let factor = $('#factorCost').val();
        let currency = $("#currencyCost").val(); //.split("-")[0].substring(1,4);
    ...
    addFunction2Array = function (f) {
        var nf = {
            graphid: (f.graphId ? f.graphId : f.graphid),
    ...
    htmlCheckBox = function (valueCostFunction, graphid, subProcessMaster, subid, checked, subprocess) {
        ...
        let activateHtml = '<div class="point-check"' + '>' +
            ...
            'description="' + valueCostFunction.description + '" ' +
