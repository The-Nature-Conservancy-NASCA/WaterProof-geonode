...
$(document).ready(function () {
  ...
  $("#add_wi").click(function () {
    ...
    if (value) {
      ...
      var action =
        "<td style='text-align:center;'><a class='btn btn-danger'><span class='glyphicon glyphicon-trash' aria-hidden='true'></span></a></td>";
      var csId = value.split("-")[1];
      if (csId != undefined) {
        var csinfras = JSON.parse(localStorage.getItem("csinfraByCity")).filter(
          (e) => e.element_system_id == csId
        );
        ...
        var intakes = JSON.parse(localStorage.getItem("intakesByCity")).filter(
          (e) => e.id == value
        );
  ...
  $("#step5NextBtn").click(function () {
    ...
    if (
      Number($("#discount").val()) < Number($("#minimum").val()) ||
      Number($("#discount").val()) > Number($("#maximum").val())
    ) {
      Swal.fire({
        icon: "warning",
  ...
  $("#step7RunBtn").click(function () {
    ...
    if ($("#period_analysis").val() != "" && $("#period_nbs").val() != "") {
      if (
        parseInt($("#period_analysis").val()) < parseInt($("#period_nbs").val())
      ) {
        Swal.fire({
          icon: "warning",
          title: gettext("field_problem"),
  ...
  function afterValidationStep7Run(valid_edit, valid_period) {
    if (
      ...
      let lbl_applied_currency = gettext(
        "The following exchange rates will be applied for the analysis"
      );
      html =
        '<div class="row" id="currencys-panel"> <div class="col-md-10 currency-panel">' +
        lbl_currency +
        '</div><div class="col-md-2 currency-panel currency-text">' +
        analysis_currency;
      html +=
        '</div><div class="col-md-12 currency-panel">' +
        lbl_applied_currency +
        ".</div>";
      html +=
        '<div class="custom-control col-md-4 currency-value">' +
        gettext("Currency") +
        "</div>";
      html +=
        '<div class="custom-control col-md-8 currency-value">' +
        gettext("Exchange") +
        "</div>";
        ...
        function (data) {
          ...
          $.each(data, function (index, currency) {
            ...
            if (currency.currency != analysis_currency) {
              value = Number.parseFloat(currency.value).toFixed(5);
              html +=
                '<div class="col-md-4 currency-value"><label class="custom-control-label" for="currency">' +
                currency.currency +
                "</label></div>";
              html +=
                '<div class="custom-control col-md-8 currency-value"><input id="' +
                currency.currency +
                '" class="text-number" type="number" class="custom-control-input" value="' +
                value +
                '"></div>';
            }
          });
          ...
          $("#full-table")
            ...
            .each(function (index, input) {
              ...
              if ($("#" + input_id).hasClass("hiddennbs")) {
                ...
                if (nbs_value > 0) valid_investment = false;
              ...
              if (result.isConfirmed) {
                ...
                $.post(
                  "/study_cases/save/",
                  {
                    ...
                    analysis_currency: $(
                      "#analysis_currency option:selected"
                    ).val(),
      ...
      success: function (json) {
        ...
        if (json.task_status == "PENDING") {
          ...
          function queryAnalisysResult() {
            ...
            if (iteration < 4) {
              console.log(
                "iteration befor 2 minutes, the process does'nt query yet"
              );
              iteration++;
              return;
              ...
              console.log(
                "iteration: " +
                  iteration +
                  ", return to list. process not finish yet..."
              );
              clearInterval(validationInterval);
        ...
        function (data) {
          ...
          if (valid_nbs) {
            Swal.fire({
              ...
              if (result.isConfirmed) {
                $("#_thumbnail_processing").modal("toggle");
                $("#_thumbnail_processing .modal-body").empty();
                let description = gettext("run_processing_description");
                let desc = document.createElement("div");
                desc.innerHTML = description;
                $("#_thumbnail_processing .modal-body").prepend(desc);
  ...
  function loadFinancialParameter() {
    $.get("/study_cases/parametersbycountry/" + cityId, function (data) {
      $.each(data, function (index, financialParameters) {
        ...
        if (!$("#evaluation").val())
          $("#evaluation").val(
            financialParameters.Monitoring_and_Evaluation_Manager_USD_YEAR
          );
        if (!$("#finance").val())
          $("#finance").val(financialParameters.Finance_Manager_USD_YEAR);
        if (!$("#implementation").val())
          $("#implementation").val(
            financialParameters.Implementation_Manager_USD_YEAR
          );
        if (!$("#office").val())
          $("#office").val(financialParameters.Office_Costs_USD_YEAR);
        if (!$("#equipment").val())
          $("#equipment").val(
            financialParameters.Equipment_Purchased_In_Year_1_USD
          );
        if (!$("#overhead").val())
        ...
        if (!$("#contracts").val()) $("#contracts").val(0);
        if (!$("#travel").val()) $("#travel").val(0);
  ...
  function getStudyCaseId(){
    const urlCloneId = window.location.pathname;  
    cloneId = urlCloneId.substring(19, urlCloneId.length);
    return cloneId
  }

  function loadNBS() {
    const cloneId=getStudyCaseId()
    if (loadedNbs) {
      autoAdjustHeight();
      return;
    }
    ...
    $.post(
      ...
      function (data) {        
        $.each(data, function (index, nbs) {
          ...
          var id = nbs.id;
          var def = nbs.default;
          var content = '<li class="list-group-item"><div class="custom-control custom-checkbox">';
          if (def) {
        ...
        loadedNbs = true;
        setTimeout(function () {
          console.log("force adjust height");
          autoAdjustHeight();
        }, 500);
      }
    );
      ...
      $.each(values, function (i, data) {
        $.each(data, function (j, intake) {
          if (intake.csinfra_elementsystem__intake__id) {
            if (
              yearsDemand.indexOf(
                intake.csinfra_elementsystem__intake__demand_parameters__years_number
              ) == -1
            ) {
              yearsDemand.push(
                intake.csinfra_elementsystem__intake__demand_parameters__years_number
            ...
            if (
              yearsDemand.indexOf(intake.demand_parameters__years_number) == -1
            ) {
  ...
  function loadNBSActivities() {
    const cloneId=getStudyCaseId()
    $.post("/study_cases/nbs/",{id_study_case: id_study_case,clone_id: cloneId,city_id: cityId, process: "Clone"}, function (data) {
      content = "";
      invesment = 0.0;
      min = 0.0;
      analysis_currency = $("#analysis_currency option:selected").val();
    ...
    Promise.all(promises).then((values) => {
      ...
      $.each(values, function (i, data) {
        $.each(data, function (j, intake) {
          if (intake.csinfra_elementsystem__intake__id) {
            if (
              listIntakes.indexOf(intake.csinfra_elementsystem__intake__id) ==
              -1
            ) {
              promise = loadBiophysical(
                intake.csinfra_elementsystem__intake__id,
                intake.csinfra_elementsystem__intake__name
              );
  ...
  function loadBiophysical(id_intake, name) {
    ...
    $.post(
      ...
      function (data) {
        ...
        content =
          '<div class="col-md-12"><legend><label>Intake ' +
          name +
          "</span> </label></legend>";
        content +=
          '<table id="bio_table_' +
          id_intake +
          '" class="table table-striped table-bordered table-condensed" style="width:100%"><thead><tr class="info">';
        content +=
          '<th scope="col" class="small text-center vat text-description-bio">description</th>';
        content += '<th scope="col" class="small text-center vat">lucode</th>';
        $.each(labels, function (key, v) {
          if (excludeKeys.indexOf(key) == -1) {
            content +=
              '<th scope="col" class="small text-center vat">' +
              gettext(key) +
              "</th>";
          }
        });
  ...
  function addCellBioparam(key, idEl, value) {
    ...
    if (bioparamValidations.hasOwnProperty(key)) {
      ...
      let step =
        bioparamValidations[key].step == undefined
          ? defaultStepBioparams
          : bioparamValidations[key].step;
  ...
  if ($("#costFunctionName").val() === "") {
    ...
    funcostdb.push({
      function: {
        ...
        currencyCostName: $("#currencyCost option:selected")
          .text()
          .trim()
          .substr(1, 3),
...
function funcost(index) {
  ...
  var currencyCostName =
    fnCost.currencyCostName != undefined
      ? fnCost.currencyCostName
      : fnCost.currency;
