...
$(document).ready(function () {
    ...
    $('#name').val(localStorage.namePortfolio);
    $('#description').val(localStorage.descPortfolio);
    ...
    $('#add_wi').click(function () {
        text = $("#select_custom_quick option:selected").text();
        value = $("#select_custom_quick option:selected").val();
        if (value) {
            $('#select_custom_quick option:selected').remove();
        ...
        if (value) {
            ...
            if (csId != undefined) {
                ...
                $.each(csinfras, function (index, intake) {
                    ...
                    var description = "<td>" + intake.description + "</td>";
                    var name_source = "<td>" + intake.water_source_name + "</td>";
                    var area_intake = "<td>" + transformArea(intake.polygon__area) + ' (Has)' + "</td>";
                    var markup = `<tr id='custom-${intake.id}-${intake.element_system_id}-${intake.graphId}'>${name} ${description} ${name_source} ${area_intake} ${action}</tr>`;
                    $("#custom_table").find('tbody').append(markup);
                });
            }else{
    ...
    $('#step6NextBtnQuick').click(function () {
        saveStepFive();
        nbs = [];
    ...
    function afterValidationStep7Run(valid_edit, valid_period){
        ...
        if ($('#period_analysis').val() != '' && $('#period_nbs').val() != '' && valid_edit && valid_period) {
            ...
            let lbl_currency = gettext('Currency for the execution this analisys_quick');
            let lbl_applied_currency = gettext('The following exchange rates will be applied for the analysis_quick');            
            ...
            html += '<div class="custom-control col-md-4 currency-value">'+ gettext('Currency_quick') +'</div>';
            html += '<div class="custom-control col-md-8 currency-value">'+ gettext('Exchange_quick') +'</div>';
            ...
            $.get("../../study_cases/currencys/", {
                ...
                $("#full-table").find("input").each(function (index, input) {
                    ...
                    if ($("#" + input_id).hasClass("hiddennbs")) {
                        ...
                        if (nbs_value < nbs_min && nbs_value > 0) {
                            ...
                            Swal.fire({
                                icon: 'warning',
                                title: gettext('field_problem_quick'),
                ...
                if (valid_investment && $('#annual_investment').val() < minimun) {
                    ...
                    Swal.fire({
                        icon: 'warning',
                        title: gettext('field_problem_quick'),
                        text: gettext('error_minimun_nbs_quick') + minimun,
                    });
                    return false
                }
                ...
                if (valid_nbs) {
                Swal.fire({
                    title: gettext('exchange_rate_quick'),
                    html: html,
                    showCancelButton: true,
                    ...
                    if (result.isConfirmed) {
                        ...
                        // let description = gettext("run_processing_description_portfolio");
                        ...
                        $.post("../../study_cases/save/", {
                            ...
                            localStorage.removeItem("ptapId");
                            localStorage.removeItem("namePortfolio");
                            localStorage.removeItem("descPortfolio");
                            localStorage.setItem("intakesByCity", null);
    ...
    function preprocRiosProcess(id_study_case,priority){
        ...
        $('#smartwizard').smartWizard("loader", "show");
        $.ajax({
            url : servermodelApi+"preproc_rios_task?id_case="+id_study_case+"%26id_usuario="+id_user+"%26priority="+priority+"%26report="+true,
            type : 'GET',
            dataType : 'json',
                        ...
                        $.ajax({
                            ...
                            success : function(json) {                                    
                                if (json.task_status == 'SUCCESS') {
                                    ...
                                    // locationHref();
                                    // Aca se debe hacer el redireccionamiento al repote rapido
                                    getReportUrl(id_study_case);
                                    clearInterval(validationInterval);
                                }
                                iteration++;                                        
                            },
    ...
    $('#custom_table').on('click', 'a', function () {
        ...
        $("#select_custom_quick").append(new Option(option, id));
        row.remove();
    ...
    function loadPtapsQuick() {
        var ptapId = localStorage.ptapId
        $("#select_ptap").empty();
        $.get("../../study_cases/ptapbyid/" + ptapId, function (data) {
            console.log(data)
            if (data.length > 0) {
                $.each(data, function (index, ptap) {
                    var name = ptap.plant_name;
    ...
    function loadIntakesQuick() {

        $("#select_custom_quick").empty();
        var intakeId = localStorage.intakeId
        $.get("../../study_cases/intakebyid/" + intakeId, function (data) {
            console.log(data)
            if (data.length > 0) {
                $.each(data, function (index, intake) {
                    var name = intake.name;
                    $("#select_custom_quick").append(new Option(name, intake.id));
                });
                $("#div-customcase").removeClass("panel-hide");
                $('#autoAdjustHeightF').css("height", "auto");
            } else {
    ...
    function loadNBSActivities() {
        var city_id = localStorage.cityId
        $.post("/study_cases/nbs/", {id_study_case: id_study_case, city_id: city_id, process: "Edit"}, function (data) {
            content = '';
            invesment = 0.0;
            min = 0.0;
            analysis_currency = $("#analysis_currency option:selected").val();        
            ...
            $.get("/study_cases/currencys/", {id: id_study_case,currency: analysis_currency}, function (currencies) {
                ...
                $('#autoAdjustHeightF').css("height", "auto");
                $('#analysis_currency')[0].selectedIndex = 210; // Cambia el índice a USD united states
                $('#analysis_currency').change(); 
            });        
        });
    }
...
function getReportUrl(id_case){
    var urlDetail = "../../reports/getWpcompareMapas/?studyCase="+ id_case;
    $.getJSON(urlDetail, function (data) {
        ...
        var urlGeographicReport = "/reports/quickGeographic/?folder="+usr_folder+"&intake="+intake+"&year="+year+"&region="+region+"&center="+centroX+"&study_case_id="+studyCaseId+"&name="+nameIntake;
        console.log (urlGeographicReport);
        localStorage.idStudyCase=studyCaseId;
        window.location.href = urlGeographicReport;
        return urlGeographicReport
    }) 
}
