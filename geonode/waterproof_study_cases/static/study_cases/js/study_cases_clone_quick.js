...
$(document).ready(function () {
    ...
    if (nbs.length > 0) {
      ...
      $.post(
        ...
        function (data) {
          ...
          $(".lbl-currency-budget").html(
            "(" + $("#analysis_currency").val() + ")"
          );
          $('#smartwizard').smartWizard("loader", "hide");
          console.log("6-step-done");
        },
        "json"
      );
    } else {
  ...
  function loadNBS() {        
    let study_case_id = window.location.href.substring(window.location.href.lastIndexOf('/') + 1);
    
    if (loadedNbs) return;
    var city_id = cityId;
    $.post("../../study_cases/nbs/", {
        id_study_case: study_case_id,
        city_id: city_id,
        process: "Quick"
    }, function (data) {
        content = '';
  ...
  function loadNBSActivities() {
    var city_id = cityId;
    $.post("/study_cases/nbs/", {id_study_case: id_study_case, city_id: city_id, process: "Quick"}, function (data) {
        content = '';
        invesment = 0.0;
        min = 0.0;
        analysis_currency = $("#analysis_currency option:selected").val();        
                ...
                if ($('#nbssc-' + id).length <= 0) {
                  ...
                  content +=  `<tr><td>${name}</td>
                                  <td><div style="display:flex;width:100%;">
                                          <input class="text-number" type="number" id="nbssc-${id}" value=${val}>
